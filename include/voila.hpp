...
struct Expression : Node
{
    ...
    bool is_table_op(bool *table_out = nullptr, bool *table_in = nullptr, bool *col = nullptr) const;
    ...
    Expression(Type type, const std::string &fun, const std::shared_ptr<Expression> &pred = nullptr) :
...
struct Statement : Node
{
  ...
  protected:
    Statement(Type t, const std::string var = "", const ExprPtr &expr = nullptr, const ExprPtr &pred = nullptr) :
        Node(Node::Type::StmtNode), type(t), var(var), expr(expr), pred(pred)
    {
    }

    Statement(Type t,
              const std::vector<std::shared_ptr<Statement>> &s,
...
{
    Loop(const ExprPtr &pred, const std::vector<std::shared_ptr<Statement>> &stms) :
