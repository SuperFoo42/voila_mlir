...
//TODO: dynamically changing signature because return type could be inferred, ignore ret type, because currently we don't do return type overload
        if (!prog->has_func(call.fun(), fType)) {
            ...
            auto clonedFun =
                    std::dynamic_pointer_cast<ast::Fun>(prog->get_func(call.fun())->clone(mapping));
        ...
        call.fun() = prog->get_func(call.fun(),
                                    fType)->name();
...
/*        for (auto &arg: fun.args()) // infer function args
    ...
    void TypeInferer::operator()(const ast::Main &main) {
        (*this)(dynamic_cast<const ast::Fun &>(main));
    }
    ...
    void TypeInferer::set_arity(const ast::ASTNode *const node, const size_t ar) {
        ...
        } else // what when returnTypes > 1 elem
        {
            std::dynamic_pointer_cast<ScalarType>(
                    types.at(std::dynamic_pointer_cast<FunctionType>(types.at(
                            typeIDs.at(node)))->returnTypeIDs.front()))
                    ->ar = Arity(ar);
        }
    }
    ...
    void TypeInferer::set_type(const ast::ASTNode *const node, const DataType type) {
        if (std::dynamic_pointer_cast<ScalarType>(get_type(*node))) {
            ...
            std::dynamic_pointer_cast<ScalarType>(
                    types.at(std::dynamic_pointer_cast<FunctionType>(types.at(
                            typeIDs.at(node)))->returnTypeIDs.front()))->t = type;
        }
    }
