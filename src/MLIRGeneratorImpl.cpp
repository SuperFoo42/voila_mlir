...
namespace voila::mlir {
    ...
    void MLIRGeneratorImpl::operator()(const FunctionCall &call) {
        ...
        result = builder.create<GenericCallOp>(location, call.fun(), operands,
                                               funcTable.lookup(call.fun()).getFunctionType().getResult(
                                                       0))->getResults();
    }
    ...
    void MLIRGeneratorImpl::operator()(FunctionCall &call) {
        ...
        result = builder.create<GenericCallOp>(location, fOp.getFunctionType().getResults(),SymbolRefAttr::get(fOp), operands).getResults();
    }
    ...
    void MLIRGeneratorImpl::operator()(const Assign &assign) {
        ...
        for (auto &en: llvm::enumerate(llvm::zip(assign.dests(), val))) {
            ...
            symbolTable.insert(
                    (dest.is_variable() ? dest.as_variable() : dest.as_reference()->ref().as_variable())->var,
                    value);
        }
    }
    ...
    void MLIRGeneratorImpl::operator()(const Gather &gather) {
        ...
        result = builder.create<GatherOp>(
                location,
                RankedTensorType::get(idx.getType().dyn_cast<TensorType>().getShape(), getElementTypeOrSelf(col)),
                col, idx, Value());
    }
